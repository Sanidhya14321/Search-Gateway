...
export default function AccountBriefPage({ params }: { params: { id: string } }) {
  ...
  if (error) return <main className="text-red-400">{error}</main>;
  if (!data) return <main className="text-stone-300">Loading account brief...</main>;

  const result = data.result || data;

  return (
    <main className="space-y-6">
      <h1 className="font-display text-3xl text-stone-100">Account Brief</h1>
      <p className="text-stone-200">{result.summary || "No summary available."}</p>
